        ...
        <div className="brand-manifesto__content w-full lg:w-1/2 flex items-center justify-center p-8 md:p-16 lg:p-24 bg-burgundy text-ivory relative">
          <div className="relative z-10 max-w-lg text-center lg:text-left">
            <motion.span
              ...
              viewport={{ once: true }}
              className="block text-gold font-sans text-xs tracking-[0.2em] uppercase mb-4"
            >
              Nuestra Filosofía
            </motion.span>
            ...
            >
              <Link
                href="/acerca-de"
                className="inline-block px-8 py-3 border border-gold text-gold hover:bg-gold hover:text-burgundy transition-all duration-300 rounded-full text-sm tracking-widest uppercase font-medium"
              >
                Nuestra Historia
              </Link>
            </motion.div>
          ...
          <div className="absolute inset-0 bg-[url('/Patron.svg')] opacity-5 pointer-events-none mix-blend-overlay"></div>
        </div>
      </div>
    </section>
  );
