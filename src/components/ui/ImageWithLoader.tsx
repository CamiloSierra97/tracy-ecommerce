...
export default function ImageWithLoader({
    ...
    return (
        <div className={twMerge("image-loader relative overflow-hidden", wrapperClassName)}>
            {isLoading && (
                <div className="image-loader__spinner-container absolute inset-0 z-10 flex items-center justify-center bg-gray-50/80 animate-pulse">
                    <ThreeRingLoader className={twMerge("image-loader__spinner w-12 h-12", loaderClassName)} />
                </div>
            )}
            <Image
                src={src}
                ...
                className={twMerge(
                    ...
                    isLoading ? "image-loader__img--loading opacity-0" : "image-loader__img--loaded opacity-100",
                    className
                )}
                onLoad={(e) => {
                    setIsLoading(false);
